    ...
    *,
    ...
    if not ids_normalizados:
        estado_busqueda = construir_estado_busqueda_local(
            ...
            parametros=parametros,
            ids_entrada=ids_consulta,
            nota="GIS requiere candidatos previos del pipeline.",
        )
        return (
            construir_estado(
                "gis",
                orden,
                "sin_resultados",
                payload={"sin_sitios": "GIS requiere candidatos previos."},
                nota="GIS requiere candidatos previos del pipeline.",
                estado_busqueda=estado_busqueda,
            ),
            tipo_ids,
    ...
    try:
        respuesta = await cliente.post_herramienta("/gis-consulta", payload)
