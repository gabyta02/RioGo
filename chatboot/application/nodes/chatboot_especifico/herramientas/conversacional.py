...
TIPOS_PERMITIDOS = {"saludo", "despedida", "agradecimiento", "emergencia"}
...
async def ejecutar(
    *,
    ...
    respuestas = _cargar_respuestas().get("chatboot_sitio") or {}
    opciones = respuestas.get(tipo) or respuestas.get("saludo") or []
    texto = (
        random.choice(opciones)
        if opciones
        else "Hola. Puedo ayudarte con información de este sitio."
    )
