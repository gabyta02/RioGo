...
Compatibilidad confirmada con 01-extensions.sql (schema turismo):
...
# turismo.tipo_contacto_t
TIPO_CONTACTO_VALIDOS = {
...
def limpiar_init_sql(sql_texto: str) -> str:
    """Omite ALTER DATABASE … SET timezone; el contenedor ya define la zona horaria."""
    lineas_limpias = []
...
def limpiar_datos_turismo(cur) -> None:
    """Vacía todas las tablas de turismo.* antes de una carga completa."""
    log.info("Limpiando datos existentes en schema turismo…")
    cur.execute(
        """
        TRUNCATE TABLE
            turismo.sitio_documento_chunk,
            turismo.sitio_documento,
            turismo.trazabilidad,
            turismo.noticia,
    ...
    log.info("Schema turismo vaciado.")
...
def cargar_contactos(cur, df: pd.DataFrame, map_sitio: dict,
                     dry_run: bool, skip_errors: bool):
    """turismo.tipo_contacto_t es un ENUM de PostgreSQL → se castea explícitamente."""
    rows = []
    for i, row in df.iterrows():
        cod = to_int(row.get("codigo_sitio"), "codigo_sitio", i)
        id_sitio = map_sitio.get(cod)
        tipo_raw = (to_str(row.get("tipo_contacto")) or "").lower()
        contenido = to_str_limit(row.get("contenido"), 200, "contenido", i)
        activo = to_bool(row.get("activo"), "activo", i)

        if id_sitio is None or not contenido:
            continue
        if tipo_raw not in TIPO_CONTACTO_VALIDOS:
            log.warning("CONTACTOS fila %d: tipo_contacto '%s' inválido → 'otro'.", i, tipo_raw)
            tipo_raw = "otro"
        rows.append((id_sitio, tipo_raw, contenido, activo if activo is not None else True))

    if dry_run:
        log.info("[DRY-RUN] CONTACTOS: %d filas listas para insertar.", len(rows))
        return

    # El ENUM requiere cast explícito en execute_values
    execute_values(
        cur,
        """
        INSERT INTO turismo.contacto (id_sitio, tipo, contenido, activo)
        VALUES %s
        """,
        rows,
        template="(%s, %s::turismo.tipo_contacto_t, %s, %s)",
    )
...
def main():
    ...
    try:
        ...
        if "no tiene creada la estructura" in str(exc) or "Faltan tablas" in str(exc):
            ...
            log.error("Alternativa: python cargar_sitios_turismo_corregido.py --init-db")
        sys.exit(1)
    finally:
        cur.close()
        conn.close()
